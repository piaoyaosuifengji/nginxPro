...
static int usbhid_parse(struct hid_device *hid)
{
	...
	//Get Report_Descriptors//取得report description的详细信息,放到rdesc中。
...
static struct hid_ll_driver usb_hid_driver = {
	.parse = usbhid_parse,/*/ 最终会调用HID框架的hid_parse_report完成report描述符的解析*/
	.start = usbhid_start,
	.stop = usbhid_stop,
	.open = usbhid_open,
	.close = usbhid_close,
...
{
	struct usb_host_interface *interface = intf->cur_altsetting;
	struct usb_device *dev = interface_to_usbdev(intf);//返回
	struct usbhid_device *usbhid
...
/*搜索借口interface中Endpoint中断in类型数量*/
	for (n = 0; n < interface->desc.bNumEndpoints; n++)
 ...
 */
	...
	if (!strlen(hid->name))
		snprintf(hid->name, sizeof(hid->name), "HID %04x:%04x",
			 ...
			 le16_to_cpu(dev->descriptor.idProduct));
    /* 
     * 填充设备结构体中的节点名。usb_make_path 用来获取 USB 设备在 Sysfs 中的路径，格式 
     * 为：usb-usb 总线号-路径名。 
     */  
...
/*	struct usb_device_id
   ...
   *
/*也就是说,该驱动会匹配interface的ClassID,所有ClassID为USB_INTERFACE_CLASS_HID的设备都会被这个驱动所匹配.
所以,所有USB HID设备都会由这个module来驱动.*/
...
static struct usb_driver hid_driver = {
	...
	.id_table =	hid_usb_ids,/*记录了当前这个驱动所能够进行驱动的那些设备的ID值*/
...
{
	int retval = -ENOMEM;
/*是一种动态进行HID设备修正的方式.具体要修正哪些设备,要修正设备的那些方面,都可以由加载模块是所带参数来决定.*/
/*经过usbhid_quirks_init()之后,所有要修正的设备的相关操作都会存放在dquirks_list中.*/
	retval = usbhid_quirks_init(quirks_param);
	if (retval)
		goto usbhid_quirks_init_fail;
	    //注册hid_driver为usb总线驱动
	retval = usb_register(&hid_driver);//调用usb_register_driver函数的宏
/*
